"""Render and dispatch one fixed FERAL study cloud request."""
...
from package_feral_coverage_cloud import encode, sha, inspect, BODY
...
def render(package, expected, binding, network):
    ...
    if plan['limits']['max_instance_seconds'] != 1800 or plan['budget']['maximum_before_tax_usd'] != '0.15':
        raise ValueError('budget differs')
    patterns = {'run_id': r'feral-49-[0-9]{8}T[0-9]{6}Z',
    ...
    values = {'W_RUN': binding['run_id'], 'W_LAUNCH': str(binding['launch_epoch_seconds']),
              ...
              'W_PACKAGE_KEY': 'packages/feral-49/' + expected + '.tar',
    ...
    for group in request['TagSpecifications']:
        for tag in group['Tags']:
            if tag['Key'] == 'Project': tag['Value'] = 'feral-49'
            if tag['Key'] == 'HostPackageSha256': tag['Key'] = 'PackageSha256'
    for group in request['TagSpecifications']:
        group['Tags'] += [{'Key': 'DeadlineEpoch', 'Value': str(binding['launch_epoch_seconds'] + 1800)}]
...
def main():
    ...
    if args.mode == 'launch':
        ...
        expected = {'package_sha256': args.package_sha256, 'plan_sha256': manifest['plan_sha256'],
                    'maximum_instance_seconds': 1800, 'maximum_before_tax_usd': '0.15',
