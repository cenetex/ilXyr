"""Controlled host failures, archive integrity and approval boundaries for weight35."""
import copy
import hashlib
import json
import os
...
from package_weight_cloud import BODY, PLAN, SOURCES, encode, sha, inspect
from weight_cloud_launch import render
from weight_cloud_collect import archive_outputs, unpack_results, receive
from weight_cloud_preflight import lifecycle_rules
from test_feral_bootstrap import STUB, write_tar

ROOT = Path(__file__).resolve().parents[1]
STUB = STUB.replace("'g6e.2xlarge'", "'c6i.4xlarge'").replace('run/model/weight.bin', 'controller/native-build/lie-2/LiE/Lie.exe').replace('run/source/grader/targets.jsonl', 'controller/COMMAND.json').replace('run/arms/base/predictions.jsonl', 'controller/corpus-run/trace.jsonl').replace('run/execution.json', 'controller/TERMINAL.json').replace("key.endswith('predictions.jsonl')", "key.endswith('results.tar')")


def fixture(root):
    plan = json.loads((ROOT / PLAN).read_bytes())
    controller_files = {'source-kit.tar': b'fixture-source-kit', 'experiments/research-step-34/CONTROLLER-PLAN.json': b'{}\n'}
    controller_files['PACKAGE.json'] = encode({'files': {n: {'bytes': len(b), 'sha256': sha(b)} for n, b in controller_files.items()}})
    controller = root / 'controller.tar'; write_tar(controller, controller_files)
    plan.update(controller_sha256=sha(controller.read_bytes()), controller_bytes=controller.stat().st_size,
        controller_plan_sha256=sha(controller_files['experiments/research-step-34/CONTROLLER-PLAN.json']), source_kit_sha256=sha(controller_files['source-kit.tar']))
    files = {n: (ROOT / n).read_bytes() for n in SOURCES}; files[PLAN] = encode(plan); files['controller.tar'] = controller.read_bytes()
    ...
    binding = {'run_id': 'weight35-20260908T000000Z', 'launch_epoch_seconds': int(time.time()), 'package_version': 'fixture-version', 'approval_reference': 'fixture-approval'}
...
def host(root, failure='', expired=False, tamper=False):
    ...
    if os.environ.get('WEIGHT_CLOUD_TEST_RECEIPTS'):
        destination = Path(os.environ['WEIGHT_CLOUD_TEST_RECEIPTS']) / ('host-' + (failure or 'success') + ('-expired' if expired else '-tampered' if tamper else ''))
    ...
    def test_success_arms_watchdog_first_collects_native_bytes_and_uses_fixed_limits(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            with tarfile.open(prefix / 'results.tar') as archive:
                self.assertEqual(archive.extractfile('controller/native-build/lie-2/LiE/Lie.exe').read(), b'fixture-model')
                self.assertEqual(archive.extractfile('controller/corpus-run/trace.jsonl').read(), b'{"id":"fixture"}\n')
...
class IntegrityTests(unittest.TestCase):
    ...
    def test_approval_preflight_and_unknown_launch_are_preserved(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            command = [sys.executable, str(ROOT / 'scripts/weight_cloud_launch.py'), 'launch', '--package', str(package), '--package-sha256', expected,
            ...
            approval = {'package_sha256': expected, 'plan_sha256': manifest['plan_sha256'], 'maximum_instance_seconds': 5400,
                'maximum_before_tax_usd': '2.00', 'approval_reference': binding['approval_reference']}
