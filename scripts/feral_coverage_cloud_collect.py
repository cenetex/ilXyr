"""Preserve bounded FERAL outputs and collect exact object versions after termination."""
import argparse
import base64
import hashlib
import io
...
def archive_outputs(root, output, max_bytes, max_members, deadline):
    ...
    with tarfile.open(output, 'x', format=tarfile.PAX_FORMAT) as archive:
        ...
        inventory = encode({'schema': 'ilxyr.feral_coverage_result_archive.v1', 'complete': complete, 'stop_reason': stop_reason, 'files': records})
...
def collect_host(root, plan, identity):
    ...
    terminal = {**identity, 'schema': 'ilxyr.feral_coverage_host_terminal.v1', 'status': 'failed',
