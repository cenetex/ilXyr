"""Render and dispatch one fixed solomon corpus cloud request."""
...
from package_solomon_cloud import encode, sha, inspect, BODY
...
def render(package, expected, binding, network):
    ...
    if provider['instance_type'] != 'c6i.large' or provider['ami_id'] != 'ami-0d3378afe7683c867':
        raise ValueError('machine differs')
    if plan['limits']['max_instance_seconds'] != 3600 or plan['budget']['maximum_before_tax_usd'] != '0.25':
        raise ValueError('budget differs')
    patterns = {'run_id': r'solomon38-[0-9]{8}T[0-9]{6}Z',
    ...
    values = {'W_RUN': binding['run_id'], 'W_LAUNCH': str(binding['launch_epoch_seconds']),
              ...
              'W_PACKAGE_KEY': 'packages/solomon38/' + expected + '.tar',
    ...
    for group in request['TagSpecifications']:
        for tag in group['Tags']:
            if tag['Key'] == 'Project': tag['Value'] = 'solomon38'
            if tag['Key'] == 'HostPackageSha256': tag['Key'] = 'PackageSha256'
    for group in request['TagSpecifications']:
        group['Tags'] += [{'Key': 'DeadlineEpoch', 'Value': str(binding['launch_epoch_seconds'] + 3600)}]
...
def main():
    ...
    if args.mode == 'launch':
        ...
        expected = {'package_sha256': args.package_sha256, 'plan_sha256': manifest['plan_sha256'],
                    'maximum_instance_seconds': 3600, 'maximum_before_tax_usd': '0.25',
