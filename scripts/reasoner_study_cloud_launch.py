"""Render and dispatch one fixed Reasoner study cloud request."""
...
from package_reasoner_study_cloud import encode, sha, inspect, BODY
...
def render(package, expected, binding, network):
    ...
    if plan['limits']['max_instance_seconds'] != 5400 or plan['budget']['maximum_before_tax_usd'] != '0.30':
        raise ValueError('budget differs')
    patterns = {'run_id': r'reasoner-46-[0-9]{8}T[0-9]{6}Z',
    ...
    values = {'W_RUN': binding['run_id'], 'W_LAUNCH': str(binding['launch_epoch_seconds']),
              ...
              'W_PACKAGE_KEY': 'packages/reasoner-46/' + expected + '.tar',
    ...
    for group in request['TagSpecifications']:
        for tag in group['Tags']:
            if tag['Key'] == 'Project': tag['Value'] = 'reasoner-46'
...
def main():
    ...
    if args.mode == 'launch':
        ...
        expected = {'package_sha256': args.package_sha256, 'plan_sha256': manifest['plan_sha256'],
                    'maximum_instance_seconds': 5400, 'maximum_before_tax_usd': '0.30',
