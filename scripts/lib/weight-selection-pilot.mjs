...
export function selectionBatch({ picker, slices, batchSize, random, state, bump,
  makeCandidate = candidateFor, sliceOffset = 0 }) {
  const candidates = [];
  ...
  try {
    while (candidates.length < batchSize && orderedSlices.length && guard++ < batchSize * 100) {
      ...
      if (!representation) { bump('no_representation', slice); continue; }
      const candidate = makeCandidate(representation, slice.desired, slice.status, random);
      if (!candidate) { bump('construction_failed', slice); continue; }
      ...
      if (state.usedQueries.has(candidate.query_key)) { bump('query_exclusions', slice); continue; }
      const orbit = orbitKey(candidate);
      if (state.orbitRestricted && state.trainingOrbits.has(orbit)) { bump('orbit_exclusions', slice); continue; }
...
export async function runSelectionPilot({ picker, random, slices: requested, query,
  oracleSha256, limits, cacheLimits, orbitRestricted = true, makeCandidate = candidateFor, rotateSlices = true }) {
  ...
  try {
    ...
    for (const candidate of pending) if (!committed.has(candidate.query_key)) release(candidate, state);
