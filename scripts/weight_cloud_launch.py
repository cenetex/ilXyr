"""Render and dispatch one fixed weight corpus cloud request."""
...
from package_weight_cloud import encode, sha, inspect, BODY
...
def render(package, expected, binding, network):
    ...
    patterns = {'run_id': r'weight35-[0-9]{8}T[0-9]{6}Z',
    ...
    values = {'W_RUN': binding['run_id'], 'W_LAUNCH': str(binding['launch_epoch_seconds']),
              ...
              'W_PACKAGE_KEY': 'packages/weight35/' + expected + '.tar',
    ...
    body = files['scripts/aws/weight35-user-data.sh']
    script = (prefix + '\n# WEIGHT35_BODY\n').encode() + body
    if len(script) > 16384:
