...
BOOT_LOG=/var/log/weight-multiplicity-phase06-preflight.log
exec > >(tee -a "$BOOT_LOG" >/dev/console) 2>&1
set -x

systemd-run --unit=phase06-preflight-emergency-shutdown --on-active=1700 \
...
PREFIX="experiments/weight-multiplicity-phase06-lie-preflight-v1/runs/${RUN_ID}"
STATUS=/tmp/weight-multiplicity-phase06-preflight-status.json
OUT=/opt/ilxyr/out
PHASE=bootstrap
TERMINAL_WRITTEN=0
SYNC_PID=
...
test "$APPROVAL_ID" = weight-multiplicity-phase06-lie-preflight-2026-08-31-v1
[[ "$RUN_ID" =~ ^[0-9]+$ ]]
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"ilxyr.weight_multiplicity_phase06_lie_preflight_cloud_status.v1",
...
timeout --signal=TERM --kill-after=30s "${remaining}s" \
  node scripts/run-weight-multiplicity-phase06-persistent-lie.mjs \
    ...
    --out "$OUT/preflight-evidence.json" \
    > "$OUT/runner-summary.json" 2> "$OUT/runner-stderr.log"
runner_status=$?
set -e
test "$runner_status" = 0 || test "$runner_status" = 2
evidence_status=$(jq -r .status "$OUT/preflight-evidence.json")
test "$evidence_status" = complete || test "$evidence_status" = hold

cd "$OUT"
sha256sum lie-build.json preflight-evidence.json runner-summary.json \
