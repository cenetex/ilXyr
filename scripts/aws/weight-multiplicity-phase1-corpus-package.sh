...
authorization=experiments/weight-multiplicity/phase1/PHASE1-CORPUS-GENERATION-AUTHORIZATION-V1.md
runner=scripts/run-weight-multiplicity-phase1-corpus.mjs

test "$(shasum -a 256 "$lie_source" | awk '{print $1}')" = \
  "$(jq -r .oracle.primary.source_sha256 "$plan")"
test "$(shasum -a 256 "$zero_source" | awk '{print $1}')" = \
  "$(jq -r .oracle.differential.source_archive_sha256 "$plan")"
test "$(shasum -a 256 "$manifest" | awk '{print $1}')" = \
  "$(jq -r .source_bindings.reduced_manifest_sha256 "$plan")"
test "$(shasum -a 256 "$systems" | awk '{print $1}')" = \
  "$(jq -r .source_bindings.root_systems_sha256 "$plan")"
test "$(shasum -a 256 "$contract" | awk '{print $1}')" = \
  "$(jq -r .source_bindings.revision3_contract_sha256 "$plan")"
test "$(shasum -a 256 "$governance" | awk '{print $1}')" = \
  "$(jq -r .source_bindings.lie_governance_sha256 "$plan")"
test "$(shasum -a 256 "$preflight" | awk '{print $1}')" = \
  "$(jq -r .source_bindings.preflight_closeout_sha256 "$plan")"
test "$(jq -r .closures.corpus_generation_authorized "$plan")" = true
test "$(jq -r .closures.model_training_authorized "$plan")" = false
...
mkdir -p "$package_dir/examples/weight-multiplicity" \
  ...
  "$package_dir/scripts"
cp "$plan" "$manifest" "$systems" "$contract" "$governance" \
...
jq -n \
  ...
  --arg zero_commit "$(jq -r .oracle.differential.source_commit "$plan")" \
  --argjson expected_records "$(jq '[.partitions[].records] | add' "$plan")" \
  '{schema:"ilxyr.weight_multiplicity_phase1_corpus_execution.v1",
    ilxyr_commit:$ilxyr_commit,plan_sha256:$plan_sha256,
    manifest_sha256:$manifest_sha256,root_systems_sha256:$systems_sha256,
    lie_source_sha256:$lie_source_sha256,zero_source_sha256:$zero_source_sha256,
    zero_commit:$zero_commit,expected_records:$expected_records,
    build_reproducibility_gate:"two_independent_lie_builds_must_match",
    corpus_generation_authorized:true,model_training_authorized:false,
    model_evaluation_authorized:false,oracle_promotion_authorized:false}' \
