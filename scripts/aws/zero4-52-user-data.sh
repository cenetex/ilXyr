...
remaining=$((W_LAUNCH + 48570 - $(date +%s)))
test "$remaining" -gt 0
systemd-run --unit=zero4-52-deadline --on-active="$remaining" /usr/sbin/shutdown -h now
ROOT=${W_WORK_ROOT:-/opt/zero4-52}
...
finish() {
  ...
  trap - EXIT
  set +e
  bounded 44420 docker inspect "$W_RUN" > "$OUT/container-inspect.json"
  bounded 44430 docker rm -f "$W_RUN"
...
PY
  if [ "$VERIFIED" = 1 ]; then
    bounded 48545 python3 "$ROOT/package/scripts/zero4_cloud_collect.py" host --root "$ROOT" \
      --plan "$ROOT/package/experiments/research-step-52/EXECUTION-PLAN.json" --identity "$ROOT/identity.json" || code=1
  else
    ...
    bounded 48500 aws s3api put-object --bucket "$W_BUCKET" --key "runs/$W_RUN/bootstrap.log" \
...
v=json.loads(Path(identity).read_bytes());v.update(schema='ilxyr.zero4_host_terminal.v1',status='failed',collection_complete=False,instance_termination_verified=False,actual_billed_usd=None)
try:
...
PYINNER
    ...
    bounded 48540 aws s3api put-object --bucket "$W_BUCKET" --key "runs/$W_RUN/host-terminal.json" \
...
plan_raw=files['experiments/research-step-52/EXECUTION-PLAN.json'];assert sha(plan_raw)==plan_sha
plan=json.loads(plan_raw);assert plan['limits']['max_instance_seconds']==48600 and plan['budget']['maximum_before_tax_usd']=='12.00'
body=Path(user_data).read_bytes().split(b'\n# ZERO4_52_BODY\n')
assert len(body)==2 and body[1]==files['scripts/aws/zero4-52-user-data.sh']
sys.path.insert(0,str(root.with_name('verified-host')/'scripts'))
from package_zero4_cloud import unpack as verify_and_unpack
verify_and_unpack(Path(archive),expected,root)
PY
VERIFIED=1
PLAN="$ROOT/package/experiments/research-step-52/EXECUTION-PLAN.json"
...
Path(path).write_text(json.dumps({'schema':'ilxyr.zero4_study_execution.v1','run_id':run,
  'package_sha256':package,'plan_sha256':plan_sha,'prepared_manifest_sha256':p['prepared']['manifest_sha256'],
  'machine':{'provider':'AWS',**{k:v[k] for k in ['region','instance_type','ami_id','architecture']},'runtime_image':p['runtime_image']},
  'limits':p['limits']},indent=2,sort_keys=True)+'\n')
PYEXEC
PHASE=image
...
python3 - "$ROOT/package" "$PLAN" "$OUT/filesystem.txt" "$OUT/DISK.json" <<'PYDISK'
import json,sys
from pathlib import Path
sys.path.insert(0,str(Path(sys.argv[1])/'scripts'))
from package_zero4_cloud import disk_receipt
plan=json.loads(Path(sys.argv[2]).read_bytes())
receipt=disk_receipt(plan,Path(sys.argv[3]).read_text())
Path(sys.argv[4]).write_text(json.dumps(receipt,sort_keys=True)+'\n')
assert receipt['passes'], 'disk space must cover output, archive, one upload part and host reserve'
PYDISK
PHASE=controller
(
  ulimit -f 32768
  bounded 44400 docker run --name "$W_RUN" --network none --cgroupns private --cpuset-cpus 0-15 --memory 24g --memory-swap 24g \
    --pids-limit 1024 --read-only --log-driver none --tmpfs /tmp:rw,exec,size=512m \
    ...
    --entrypoint python3 "$IMAGE" /work/package/scripts/zero4_cloud_runtime.py cloud \
