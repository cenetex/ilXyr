...
test "$remaining" -gt 0
systemd-run --unit=reasoner-46-deadline --on-active="$remaining" /usr/sbin/shutdown -h now
ROOT=${W_WORK_ROOT:-/opt/reasoner-46}
...
finish() {
  ...
  trap - EXIT
  set +e
  bounded 4250 docker inspect "$W_RUN" > "$OUT/container-inspect.json"
  bounded 4260 docker rm -f "$W_RUN"
...
PY
  if [ "$VERIFIED" = 1 ]; then
    bounded 5345 python3 "$ROOT/package/scripts/reasoner_study_cloud_collect.py" host --root "$ROOT" \
      --plan "$ROOT/package/experiments/research-step-46/EXECUTION-PLAN.json" --identity "$ROOT/identity.json" || code=1
  else
...
v=json.loads(Path(identity).read_bytes());v.update(schema='ilxyr.reasoner_host_terminal.v1',status='failed',collection_complete=False,instance_termination_verified=False,actual_billed_usd=None)
try:
...
plan_raw=files['experiments/research-step-46/EXECUTION-PLAN.json'];assert sha(plan_raw)==plan_sha
plan=json.loads(plan_raw);assert plan['limits']['max_instance_seconds']==5400 and plan['budget']['maximum_before_tax_usd']=='0.30'
body=Path(user_data).read_bytes().split(b'\n# REASONER_46_BODY\n')
assert len(body)==2 and body[1]==files['scripts/aws/reasoner-46-user-data.sh']
sys.path.insert(0,str(root.with_name('verified-host')/'scripts'))
from package_reasoner_study_cloud import unpack as verify_and_unpack
verify_and_unpack(Path(archive),expected,root)
PY
VERIFIED=1
PLAN="$ROOT/package/experiments/research-step-46/EXECUTION-PLAN.json"
...
Path(path).write_text(json.dumps({'schema':'ilxyr.reasoner_study_execution.v1','venue':'cloud','run_id':run,
  'package_sha256':package,'plan_sha256':plan_sha,'prepared_bindings_sha256':p['prepared']['bindings_sha256'],
  'implementation':p['implementation'], 'compiler_flags':p['compiler_flags'], 'worker_count':1,
  'compiler_identity':p['compiler_identity'], 'node_identity':p['node_identity'],
...
(
  ...
  bounded 4230 docker run --name "$W_RUN" --network none --cgroupns private --cpuset-cpus 0 --memory 3g --memory-swap 3g \
    ...
    --env PATH=/work/package/runtime/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin --env CC=/usr/bin/clang --env NODE_OPTIONS=--max-old-space-size=1536 \
    ...
    --entrypoint python3 "$IMAGE" /work/package/scripts/reasoner_study_cloud_runtime.py cloud \
