...
BOOT_LOG=/var/log/weight-multiplicity-phase1-corpus.log
exec > >(tee -a "$BOOT_LOG" >/dev/console) 2>&1
set -x

systemd-run --unit=phase1-corpus-emergency-shutdown --on-active=5300 \
...
PREFIX="experiments/weight-multiplicity-phase1-corpus-v1/runs/${RUN_ID}"
STATUS=/tmp/weight-multiplicity-phase1-corpus-status.json
OUT=/opt/ilxyr/out
PHASE=bootstrap
TERMINAL_WRITTEN=0
SYNC_PID=
...
test "$MAX_INSTANCE_SECONDS" = 5400
test "$WORKLOAD_TIMEOUT_SECONDS" = 4800
test "$MAX_COMPUTE_USD" = 1.02
test "$HOURLY_PRICE" = 0.68
test "$APPROVAL_ID" = weight-multiplicity-phase1-corpus-2026-08-31-v1
[[ "$RUN_ID" =~ ^[0-9]+$ ]]
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"ilxyr.weight_multiplicity_phase1_corpus_cloud_status.v1",
...
sync_outputs() {
  test -d "$OUT" || return 0
  aws s3 sync "$OUT/" "s3://${BUCKET}/${PREFIX}/state/" \
    --exclude '*.ndjson' --only-show-errors
}
finish() {
  exit_code=$?
  trap - EXIT
...
test "$(jq -r .expected_records execution-record.json)" = 458750
test "$(jq -r .corpus_generation_authorized execution-record.json)" = true
...
jq -n \
  ...
  '{schema:"ilxyr.weight_multiplicity_phase1_oracle_builds.v1",
...
result_status=$(jq -r .status "$OUT/generation/runner-summary.json")
test "$result_status" = corpus_complete || test "$result_status" = hold
if [ "$result_status" = corpus_complete ]; then
  test "$(jq -r .total_records "$OUT/generation/corpus-manifest.json")" = 458750
  test "$(jq -r .zero_differential.disagreements "$OUT/generation/corpus-manifest.json")" = 0
  test "$(jq -r .closures.model_training_authorized "$OUT/generation/corpus-manifest.json")" = false
  cd "$OUT/generation"
