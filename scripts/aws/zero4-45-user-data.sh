...
test "$remaining" -gt 0
systemd-run --unit=zero4-45-deadline --on-active="$remaining" /usr/sbin/shutdown -h now
ROOT=${W_WORK_ROOT:-/opt/zero4-45}
...
PY
  if [ "$VERIFIED" = 1 ]; then
    bounded 48545 python3 "$ROOT/package/scripts/zero4_cloud_collect.py" host --root "$ROOT" \
      --plan "$ROOT/package/experiments/research-step-45/EXECUTION-PLAN.json" --identity "$ROOT/identity.json" || code=1
  else
...
plan_raw=files['experiments/research-step-45/EXECUTION-PLAN.json'];assert sha(plan_raw)==plan_sha
...
assert len(body)==2 and body[1]==files['scripts/aws/zero4-45-user-data.sh']
...
PY
VERIFIED=1
PLAN="$ROOT/package/experiments/research-step-45/EXECUTION-PLAN.json"
