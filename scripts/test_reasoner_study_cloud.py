"""Controlled host failures, archive integrity and approval boundaries for reasoner-46."""
import copy
import hashlib
import json
import os
...
from package_reasoner_study_cloud import BODY, PLAN, SOURCES, KIT, encode, sha, inspect
from reasoner_study_cloud_launch import render
from reasoner_study_cloud_collect import archive_outputs, unpack_results, receive
from reasoner_study_cloud_preflight import lifecycle_rules
...
def fixture(root):
    ...
    implementation = {'research_reasoner_study.py': sha(controller_files['scripts/research_reasoner_study.py'])}
    ...
    prepared_files = {'source/fixture': b'controlled inputs', 'PLAN.json': b'{}', 'ROSTER.json': b'[]', 'OPENED.jsonl': b'',
                      'opened-SCHEDULE.json': b'{}', 'cloud-SCHEDULE.json': b'{}'}
    prepared_files['BINDINGS.json'] = encode({'source': {'fixture': sha(prepared_files['source/fixture'])},
        'plan_sha256': sha(prepared_files['PLAN.json']), 'roster_sha256': sha(prepared_files['ROSTER.json']),
        'opened_sha256': sha(prepared_files['OPENED.jsonl']), 'implementation': implementation})
    ...
    binding = {'run_id': 'reasoner-46-20260912T000000Z', 'launch_epoch_seconds': int(time.time()), 'package_version': 'fixture-version', 'approval_reference': 'fixture-approval'}
...
def host(root, failure='', expired=False, tamper=False):
    ...
    if os.environ.get('REASONER_CLOUD_TEST_RECEIPTS'):
        destination = Path(os.environ['REASONER_CLOUD_TEST_RECEIPTS']) / ('host-' + (failure or 'success') + ('-expired' if expired else '-tampered' if tamper else ''))
...
class HostTests(unittest.TestCase):
    def test_success_arms_watchdog_first_collects_native_bytes_and_uses_fixed_limits(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            self.assertTrue(5300 < timer <= 5370)
...
class IntegrityTests(unittest.TestCase):
    ...
    def test_receive_requires_termination_before_s3_calls(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            with patch('reasoner_study_cloud_collect.aws', return_value=response) as call, self.assertRaisesRegex(ValueError, 'termination required'):
    ...
    def test_approval_preflight_and_unknown_launch_are_preserved(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            runner = ROOT / 'scripts/reasoner_study_cloud_launch.py'
            ...
            approval = {'package_sha256': expected, 'plan_sha256': manifest['plan_sha256'], 'maximum_instance_seconds': 5400,
                'maximum_before_tax_usd': '0.30', 'approval_reference': binding['approval_reference']}
            ...
            preflight['run_id'] = 'reasoner-46-20260911T000000Z'
            ...
            if os.environ.get('REASONER_CLOUD_TEST_RECEIPTS'):
                destination = Path(os.environ['REASONER_CLOUD_TEST_RECEIPTS']) / 'changed-preflight-run'
    ...
    def test_collection_checks_exact_versions_hashes_and_cleanup(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            def fake_aws(args, deadline, profile):
                ...
                return metadata
            with patch('reasoner_study_cloud_collect.aws', side_effect=fake_aws):
    ...
    def test_lifecycle_only_covers_new_study_folders(self):
        ...
        self.assertEqual([v['Filter']['Prefix'] for v in rules[:2]], ['packages/reasoner-46/', 'runs/reasoner-46-'])
        for rule in rules[:2]:
...
class ChunkTests(unittest.TestCase):
    def test_ordered_parts_bind_exact_coverage(self):
        from reasoner_study_cloud_collect import upload_parts, validate_parts
        with tempfile.TemporaryDirectory() as name:
            ...
            with patch('reasoner_study_cloud_collect.put', side_effect=put):
    ...
    def test_failed_second_upload_keeps_first_binding_and_partial_bytes(self):
        from reasoner_study_cloud_collect import upload_parts
        with tempfile.TemporaryDirectory() as name:
            ...
            with patch('reasoner_study_cloud_collect.put', side_effect=put), self.assertRaisesRegex(RuntimeError, 'injected upload'):
                upload_parts(bundle, storage, 'runs/fixture/', time.time() + 5, receipt, root / 'collection.json')
    ...
    def test_cost_ceiling_rejects_an_omitted_download_charge(self):
        from package_reasoner_study_cloud import check_budget
...
class RuntimeTests(unittest.TestCase):
    def test_changed_memory_limit_stops_before_full_controller(self):
        from package_reasoner_study_cloud import unpack
        import reasoner_study_cloud_runtime as runtime
        with tempfile.TemporaryDirectory() as name:
            ...
            execution = {'schema': 'ilxyr.reasoner_study_execution.v1', 'plan_sha256': sha(encode(plan)),
            ...
            with patch.dict(os.environ, plan['environment']), patch.object(runtime.platform, 'machine', return_value='x86_64'), \
                 patch.object(runtime.subprocess, 'check_output', side_effect=[plan['compiler_identity'], plan['python_identity'], plan['node_identity']]), \
