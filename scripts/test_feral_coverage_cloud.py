"""Controlled host failures, archive integrity and approval boundaries for feral-49."""
import copy
import hashlib
import json
import os
...
from package_feral_coverage_cloud import BODY, PLAN, SOURCES, KIT, encode, sha, inspect
from feral_coverage_cloud_launch import render
from feral_coverage_cloud_collect import archive_outputs, unpack_results, receive
from feral_coverage_cloud_preflight import lifecycle_rules
...
def fixture(root):
    ...
    controller_files = {'scripts/research_feral_coverage.py': b'# controlled source fixture\n', 'experiments/research-step-48/PLAN.json': b'{}'}
    ...
    implementation = {'research_feral_coverage.py': sha(controller_files['scripts/research_feral_coverage.py'])}
    kit = {'archive_sha256': sha(controller.read_bytes()), 'archive_bytes': controller.stat().st_size,
           'manifest_sha256': sha(controller_files['FILES.json']), 'implementation': implementation}
    prepared_files = {'PLAN.json': b'{}', 'FIXTURES.json': b'[]', 'fresh/PREPARE.json': b'{}',
                      'raw/fixture.html': b'controlled report', 'opened-SCHEDULE.json': b'{}', 'cloud-SCHEDULE.json': b'{}'}
    prepared_files['BINDINGS.json'] = encode({'files': {n: {'bytes': len(b), 'sha256': sha(b)} for n, b in prepared_files.items()},
        'plan_sha256': sha(prepared_files['PLAN.json']), 'fixtures_sha256': sha(prepared_files['FIXTURES.json']),
        'fresh_prepare_sha256': sha(prepared_files['fresh/PREPARE.json']), 'raw_sources_present': True,
        'implementation': implementation})
    kit['prepared_bindings_sha256'] = sha(prepared_files['BINDINGS.json'])
    prepared = root / 'prepared.tar'; write_tar(prepared, prepared_files)
    plan.update(controller_sha256=kit['archive_sha256'], controller_bytes=kit['archive_bytes'], implementation=implementation,
        study_plan_sha256=sha(prepared_files['PLAN.json']),
    ...
    binding = {'run_id': 'feral-49-20260912T000000Z', 'launch_epoch_seconds': int(time.time()), 'package_version': 'fixture-version', 'approval_reference': 'fixture-approval'}
...
def host(root, failure='', expired=False, tamper=False):
    ...
    if os.environ.get('FERAL_COVERAGE_CLOUD_TEST_RECEIPTS'):
        destination = Path(os.environ['FERAL_COVERAGE_CLOUD_TEST_RECEIPTS']) / ('host-' + (failure or 'success') + ('-expired' if expired else '-tampered' if tamper else ''))
...
class HostTests(unittest.TestCase):
    def test_success_arms_watchdog_first_collects_exact_bytes_and_uses_fixed_limits(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            self.assertTrue(1700 < timer <= 1770)
            docker = next(v['args'] for v in calls if v['name'] == 'docker' and v['args'][0] == 'run')
            for option, value in [('--memory', '3g'), ('--memory-swap', '3g'), ('--cpuset-cpus', '0'), ('--network', 'none'), ('--pids-limit', '256')]:
            ...
            with tarfile.open(prefix / 'results-00.part') as archive:
                self.assertEqual(archive.extractfile('runtime/study/inputs/fixture.bin').read(), b'fixture-model')
...
class IntegrityTests(unittest.TestCase):
    def test_node_archive_requires_fixed_bytes_and_bounded_regular_binary(self):
        from package_feral_coverage_cloud import node_bytes, read_archive
        with tempfile.TemporaryDirectory() as name:
    ...
    def test_prepared_source_change_is_rejected_with_rebound_outer_archive(self):
        from package_feral_coverage_cloud import read_archive
        with tempfile.TemporaryDirectory() as name:
            root = Path(name); package, _, _, _ = fixture(root)
            files = read_archive(package.read_bytes()); prepared = read_archive(files['prepared.tar'])
            prepared['raw/fixture.html'] += b' changed'
            write_tar(root / 'changed-prepared.tar', prepared)
            files['prepared.tar'] = (root / 'changed-prepared.tar').read_bytes()
            plan = json.loads(files[PLAN])
            plan['prepared'].update(bytes=len(files['prepared.tar']), sha256=sha(files['prepared.tar']))
    ...
    def test_receive_requires_termination_before_s3_calls(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            with patch('feral_coverage_cloud_collect.aws', return_value=response) as call, self.assertRaisesRegex(ValueError, 'termination required'):
    ...
    def test_approval_preflight_and_unknown_launch_are_preserved(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            runner = ROOT / 'scripts/feral_coverage_cloud_launch.py'
            ...
            approval = {'package_sha256': expected, 'plan_sha256': manifest['plan_sha256'], 'maximum_instance_seconds': 1800,
                'maximum_before_tax_usd': '0.15', 'approval_reference': binding['approval_reference']}
            ...
            preflight['run_id'] = 'feral-49-20260911T000000Z'
            ...
            if os.environ.get('FERAL_COVERAGE_CLOUD_TEST_RECEIPTS'):
                destination = Path(os.environ['FERAL_COVERAGE_CLOUD_TEST_RECEIPTS']) / 'changed-preflight-run'
    ...
    def test_collection_checks_exact_versions_hashes_and_cleanup(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            def fake_aws(args, deadline, profile):
                ...
                return metadata
            with patch('feral_coverage_cloud_collect.aws', side_effect=fake_aws):
    ...
    def test_lifecycle_only_covers_new_study_folders(self):
        ...
        self.assertEqual([v['Filter']['Prefix'] for v in rules[:2]], ['packages/feral-49/', 'runs/feral-49-'])
        for rule in rules[:2]:
...
class ChunkTests(unittest.TestCase):
    def test_ordered_parts_bind_exact_coverage(self):
        from feral_coverage_cloud_collect import upload_parts, validate_parts
        with tempfile.TemporaryDirectory() as name:
            ...
            with patch('feral_coverage_cloud_collect.put', side_effect=put):
    ...
    def test_failed_second_upload_keeps_first_binding_and_partial_bytes(self):
        from feral_coverage_cloud_collect import upload_parts
        with tempfile.TemporaryDirectory() as name:
            ...
            with patch('feral_coverage_cloud_collect.put', side_effect=put), self.assertRaisesRegex(RuntimeError, 'injected upload'):
                upload_parts(bundle, storage, 'runs/fixture/', time.time() + 5, receipt, root / 'collection.json')
    ...
    def test_cost_ceiling_rejects_an_omitted_download_charge(self):
        from package_feral_coverage_cloud import check_budget
...
class RuntimeTests(unittest.TestCase):
    def test_changed_memory_limit_stops_before_full_controller(self):
        from package_feral_coverage_cloud import unpack
        import feral_coverage_cloud_runtime as runtime
        with tempfile.TemporaryDirectory() as name:
            root = Path(name); package, _, _, _ = fixture(root)
            unpack(package, sha(package.read_bytes()), root / 'package')
            plan = json.loads((root / 'package' / PLAN).read_bytes()); provider = plan['provider']
            execution = {'schema': 'ilxyr.feral_coverage_execution.v1', 'plan_sha256': sha(encode(plan)),
                'prepared_bindings_sha256': plan['prepared']['bindings_sha256'], 'host_limits': plan['limits'], 'venue': 'cloud',
                'machine': {'provider': 'AWS', **{k: provider[k] for k in ['region', 'instance_type', 'architecture']},
                            'image_id': provider['ami_id'], 'runtime_image': plan['runtime_image']}}
            ...
            with patch.dict(os.environ, plan['environment']), patch.object(runtime.platform, 'machine', return_value='x86_64'), \
                 patch.object(runtime.subprocess, 'check_output', side_effect=[plan['python_identity'], plan['node_identity']]), \
