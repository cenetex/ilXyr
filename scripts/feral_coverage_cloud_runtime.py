"""Run the fixed FERAL study and its independent checker in the offline image."""
import argparse
import json
import os
...
import sys
import time
from package_feral_coverage_cloud import PLAN, encode, require, sha
...
def run(package, output, mode, execution):
    ...
    record = {'schema': 'ilxyr.feral_coverage_runtime.v1', 'status': 'failed', 'mode': mode, 'phase': 'identity'}
    ...
    try:
        ...
        for key, command in [('python_identity', [sys.executable, '--version']), ('node_identity', ['node', '--version'])]:
            record[key] = subprocess.check_output(command, text=True, timeout=10).strip()
        record['python_identity'] = record['python_identity'].removeprefix('Python ')
        for key in ['python_identity', 'node_identity']:
        ...
        if mode == 'cloud':
            ...
            require(e['schema'] == 'ilxyr.feral_coverage_execution.v1' and e['venue'] == 'cloud' and
            ...
            require(e['limits'] == plan['study_limits'] and e['implementation'] == plan['implementation'] and
                    e['worker_count'] == 1 and
                    all(e[k] == plan[k] for k in ['python_identity', 'node_identity']), 'runtime study settings differ')
            require(re.fullmatch(r'[a-f0-9]{64}', e['package_sha256']) and re.fullmatch(r'feral-49-[0-9]{8}T[0-9]{6}Z', e['run_id']), 'runtime package binding differs')
            record['execution_sha256'] = sha(execution.read_bytes())
        else:
            require(execution is None, 'opened check uses its fixed inputs')
        command = [sys.executable, str(kit / 'scripts/research_feral_coverage.py'), mode,
        ...
        command = [sys.executable, str(kit / 'scripts/check_feral_coverage.py'), '--prepared', str(prepared),
        ...
        check = json.loads((output / 'verified/verified/CHECK.json').read_bytes())
        require(check['status'] == 'verified_complete' and check['mode'] == mode, 'study coverage differs')
        record.update(status='complete', phase='complete', check_sha256=sha((output / 'verified/verified/CHECK.json').read_bytes()),
                      fresh_predictor_calls=check['fresh_predictor_calls'], distinct_results=check['distinct_results'],
